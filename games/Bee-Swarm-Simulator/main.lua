...
function farm(trying)
    if sleepy.toggles.loopfarmspeed then game.PlayersLocalPlayer.Character.Humanoid.WalkSpeed = sleepy.vars.farmspeed end
...
function gettoken(v3)
    ...
    for e,r in next, game:GetService("Workspace").Collectibles:GetChildren() do
        if tonumber((r.Position - game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) <= temptable.magnitude/1.4 and (v3-r.Position).magnitude <= temptable.magnitude then
            farm(r)
        end
    end
end
        ...
        for e,r in next, game:GetService("Workspace").Collectibles:GetChildren() do
            if r:FindFirstChildOfClass("Decal") then
                ...
                if aaaaaaaa ~= nil and sleepy.findvalue(sleepy.priority, aaaaaaaa) then
                    if r.Name == game.Players.LocalPlayer.Name and not r:FindFirstChild("got it") or tonumber((r.Position - game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) <= temptable.magnitude/1.4 and not r:FindFirstChild("got it") then
...
function converthoney()
    ...
    if temptable.converting then
        if game.Players.LocalPlayer.PlayerGui.ScreenGui.ActivateButton.TextBox.Text ~= "Stop Making Honey" and game.Players.LocalPlayer.PlayerGui.ScreenGui.ActivateButton.BackgroundColor3 ~= Color3.new(201, 39, 28) or (game:GetService("Players").LocalPlayer.SpawnPos.Value.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude > 10 then
...
function closestleaf()
    for i,v in next, game.Workspace.Flowers:GetChildren() do
        if temptable.running == false and tonumber((v.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
            farm(v)
            break
        end
    end
...
function getbubble()
    for i,v in next, game.workspace.Particles:GetChildren() do
        if string.find(v.Name, "Bubble") and temptable.running == false and tonumber((v.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
            farm(v)
            break
        end
    end
    ...
    for i,v in next, game:GetService("Workspace").Balloons.FieldBalloons:GetChildren() do
        if v:FindFirstChild("BalloonRoot") and v:FindFirstChild("PlayerName") then
            if v:FindFirstChild("PlayerName").Value == game.Players.LocalPlayer.Name then
                if tonumber((v.BalloonRoot.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
...
function getflower()
    ...
    if tonumber((flowerrrr-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) <= temptable.magnitude/1.4 and tonumber((flowerrrr-fieldposition).magnitude) <= temptable.magnitude/1.4 then 
        if temptable.running == false then 
            if sleepy.toggles.loopfarmspeed then 
                game.PlayersLocalPlayer.Character.Humanoid.WalkSpeed = sleepy.vars.farmspeed 
            end 
...
function getcloud()
    for i,v in next, game:GetService("Workspace").Clouds:GetChildren() do
        ...
        if e and tonumber((e.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
...
function getfuzzy()
    pcall(function()
        for i,v in next, game.workspace.Particles:GetChildren() do
            if v.Name == "DustBunnyInstance" and temptable.running == false and tonumber((v.Plane.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
...
function getflame()
    for i,v in next, game:GetService("Workspace").PlayerFlames:GetChildren() do
        if tonumber((v.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
            farm(v)
            break
        end
    end
...
function avoidmob()
    for i,v in next, game:GetService("Workspace").Monsters:GetChildren() do
        if v:FindFirstChild("Head") then
            if (v.Head.Position-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude < 30 and sleepy.humanoid():GetState() ~= Enum.HumanoidStateType.Freefall then
                game.PlayersLocalPlayer.Character.Humanoid.Jump = true
            end
        end
    end
end
        ...
        if v.Name ~= "Ant Challenge Info" and v.Name ~= "Bubble Bee Man 2" and v.Name ~= "Wind Shrine" and v.Name ~= "Gummy Bear" then if v:FindFirstChild("Platform") then if v.Platform:FindFirstChild("AlertPos") then if v.Platform.AlertPos:FindFirstChild("AlertGui") then if v.Platform.AlertPos.AlertGui:FindFirstChild("ImageLabel") then
            ...
            if image.ImageTransparency == 0 then
                if sleepy.toggles.tptonpc then
                    game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(v.Platform.Position.X, v.Platform.Position.Y+3, v.Platform.Position.Z)
                    task.wait(1)
                else
...
wayp:CreateDropdown("Field Teleports", fieldstable, function(Option) game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").FlowerZones:FindFirstChild(Option).CFrame end)
wayp:CreateDropdown("Monster Teleports", spawnerstable, function(Option) d = game:GetService("Workspace").MonsterSpawners:FindFirstChild(Option) game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(d.Position.X, d.Position.Y+3, d.Position.Z) end)
wayp:CreateDropdown("Toys Teleports", toystable, function(Option) d = game:GetService("Workspace").Toys:FindFirstChild(Option).Platform game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(d.Position.X, d.Position.Y+3, d.Position.Z) end)
wayp:CreateButton("Teleport to hive", function() game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.SpawnPos.Value end)


local miscc = misctab:CreateSection("Misc")
miscc:CreateButton("Ant Challenge Semi-Godmode", function() sleepy.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) task.wait(1) game.ReplicatedStorage.Events.ToyEvent:FireServer("Ant Challenge") game.PlayersLocalPlayer.Character.HumanoidRootPart.Position = Vector3.new(93.4228, 42.3983, 553.128) task.wait(2) game.PlayersLocalPlayer.Character.Humanoid.Name = 1 local l = game.PlayersLocalPlayer.Character["1"]:Clone() l.Parent = game.PlayersLocalPlayer.Character l.Name = "Humanoid" task.wait() game.PlayersLocalPlayer.Character["1"]:Destroy() sleepy.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) task.wait(8) sleepy.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) end)
...
extras:CreateButton("Invisibility", function(State) sleepy.teleport(CFrame.new(0,0,0)) wait(1) if game.PlayersLocalPlayer.Character:FindFirstChild('LowerTorso') then Root = game.PlayersLocalPlayer.Character.LowerTorso.Root:Clone() game.PlayersLocalPlayer.Character.LowerTorso.Root:Destroy() Root.Parent = game.PlayersLocalPlayer.Character.LowerTorso sleepy.teleport(game:GetService("Players").LocalPlayer.SpawnPos.Value) end end)
...
task.spawn(function() while task.wait() do
    if sleepy.toggles.autofarm then
        ...
        if game.PlayersLocalPlayer.Character:FindFirstChild("ProgressLabel",true) then
        local pollenprglbl = game.PlayersLocalPlayer.Character:FindFirstChild("ProgressLabel",true)
        ...
        if tonumber(pollenpercentage) < tonumber(sleepy.vars.convertat) then
            if not temptable.tokensfarm then
                ...
                if (fieldposition-game.PlayersLocalPlayer.Character.HumanoidRootPart.Position).magnitude > temptable.magnitude then
                    sleepy.tween(2, fieldpos)
                    task.wait(2)
                    if sleepy.toggles.autosprinkler then makesprinklers() end
                end
...
task.spawn(function() while task.wait(0.001) do
    if sleepy.toggles.traincrab then game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-259, 111.8, 496.4) * CFrame.fromEulerAnglesXYZ(0, 110, 90) temptable.float = true temptable.float = false end
    if sleepy.toggles.farmrares then for k,v in next, game.workspace.Collectibles:GetChildren() do if v.CFrame.YVector.Y == 1 then if v.Transparency == 0 then decal = v:FindFirstChildOfClass("Decal") for e,r in next, sleepy.rares do if decal.Texture == r or decal.Texture == "rbxassetid://"..r then game.PlayersLocalPlayer.Character.HumanoidRootPart.CFrame = v.CFrame break end end end end end end
    if sleepy.toggles.autodig then workspace.NPCs.Onett.Onett["Porcelain Dipper"].ClickEvent:FireServer() if game.Players.LocalPlayer then if game.PlayersLocalPlayer.Character then if game.PlayersLocalPlayer.Character:FindFirstChildOfClass("Tool") then if game.PlayersLocalPlayer.Character:FindFirstChildOfClass("Tool"):FindFirstChild("ClickEvent", true) then clickevent = game.PlayersLocalPlayer.Character:FindFirstChildOfClass("Tool"):FindFirstChild("ClickEvent", true) or nil end end end if clickevent then clickevent:FireServer() end end end
end end)
...
game:GetService('RunService').Heartbeat:connect(function() 
    ...
    if sleepy.toggles.loopspeed then game.PlayersLocalPlayer.Character.Humanoid.WalkSpeed = sleepy.vars.walkspeed end
    if sleepy.toggles.loopjump then game.PlayersLocalPlayer.Character.Humanoid.JumpPower = sleepy.vars.jumppower end
end)
...
game:GetService('RunService').Heartbeat:connect(function() 
    if temptable.float then game.PlayersLocalPlayer.Character.Humanoid.BodyTypeScale.Value = 0 floatpad.CanCollide = true floatpad.CFrame = CFrame.new(game.PlayersLocalPlayer.Character.HumanoidRootPart.Position.X, game.PlayersLocalPlayer.Character.HumanoidRootPart.Position.Y-3.75, game.PlayersLocalPlayer.Character.HumanoidRootPart.Position.Z) task.wait(0)  else floatpad.CanCollide = false end
end)
