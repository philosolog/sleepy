...
function farm(trying)
    if sleepy.toggles.loopfarmspeed then game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = sleepy.vars.farmspeed end
...
function gettoken(v3)
    ...
    for e,r in next, game:GetService("Workspace").Collectibles:GetChildren() do
        if tonumber((r.Position - game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) <= temptable.magnitude/1.4 and (v3-r.Position).magnitude <= temptable.magnitude then
            farm(r)
        end
    end
end
...
function makesprinklers()
    ...
    for i = 1, e do
        k = sleepy.humanoid().JumpPower
        if e ~= 1 then sleepy.humanoid().JumpPower = 70 sleepy.humanoid().Jump = true task.wait(.2) end
        game.ReplicatedStorage.Events.PlayerActivesCommand:FireServer({["Name"] = "Sprinkler Builder"})
        if e ~= 1 then sleepy.humanoid().JumpPower = k task.wait(1) end
    end
end
    ...
    for i,v in pairs(game:GetService("Workspace").MonsterSpawners:GetChildren()) do
        if v:FindFirstChild("Territory") then
            if v.Name ~= "Commando Chick" and v.Name ~= "CoconutCrab" and v.Name ~= "StumpSnail" and v.Name ~= "TunnelBear" and v.Name ~= "King Beetle Cave" and not v.Name:match("CaveMonster") and not v:FindFirstChild("TimerLabel", true).Visible then
                ...
                sleepy.humanoidrootpart().CFrame = monsterpart.CFrame
                repeat sleepy.humanoidrootpart().CFrame = monsterpart.CFrame avoidmob() task.wait(1) until v:FindFirstChild("TimerLabel", true).Visible
...
function farmant()
    ...
    task.wait(1)
    game.ReplicatedStorage.Events.PlayerActivesCommand:FireServer({["Name"] = "Sprinkler Builder"})
    sleepy.humanoidrootpart().CFrame = sleepy.humanoidrootpart().CFrame + Vector3.new(0, 15, 0)
    task.wait(3)
    repeat
        task.wait()
        for i,v in next, game.Workspace.Toys["Ant Challenge"].Obstacles:GetChildren() do
            if v:FindFirstChild("Root") then
                if (v.Root.Position-sleepy.humanoidrootpart().Position).magnitude <= 40 and anttable.left then
                    sleepy.humanoidrootpart().CFrame = acr
                    anttable.left = false anttable.right = true
                    wait(.1)
                elseif (v.Root.Position-sleepy.humanoidrootpart().Position).magnitude <= 40 and anttable.right then
                    sleepy.humanoidrootpart().CFrame = acl
...
function collectplanters()
    ...
    for i,v in pairs(planterst.plantername) do
        if sleepy.partwithnamepart(v, game:GetService("Workspace").Planters) and sleepy.partwithnamepart(v, game:GetService("Workspace").Planters):FindFirstChild("Soil") then
            soil = sleepy.partwithnamepart(v, game:GetService("Workspace").Planters).Soil
            sleepy.humanoidrootpart().CFrame = soil.CFrame
        ...
        for e,r in next, game:GetService("Workspace").Collectibles:GetChildren() do
            if r:FindFirstChildOfClass("Decal") then
                ...
                if aaaaaaaa ~= nil and sleepy.findvalue(sleepy.priority, aaaaaaaa) then
                    if r.Name == game.Players.LocalPlayer.Name and not r:FindFirstChild("got it") or tonumber((r.Position - game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) <= temptable.magnitude/1.4 and not r:FindFirstChild("got it") then
...
function converthoney()
    ...
    if temptable.converting then
        if game.Players.LocalPlayer.PlayerGui.ScreenGui.ActivateButton.TextBox.Text ~= "Stop Making Honey" and game.Players.LocalPlayer.PlayerGui.ScreenGui.ActivateButton.BackgroundColor3 ~= Color3.new(201, 39, 28) or (game:GetService("Players").LocalPlayer.SpawnPos.Value.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 10 then
...
function closestleaf()
    for i,v in next, game.Workspace.Flowers:GetChildren() do
        if temptable.running == false and tonumber((v.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
            farm(v)
            break
        end
    end
...
function getbubble()
    for i,v in next, game.workspace.Particles:GetChildren() do
        if string.find(v.Name, "Bubble") and temptable.running == false and tonumber((v.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
            farm(v)
            break
        end
    end
    ...
    for i,v in next, game:GetService("Workspace").Balloons.FieldBalloons:GetChildren() do
        if v:FindFirstChild("BalloonRoot") and v:FindFirstChild("PlayerName") then
            if v:FindFirstChild("PlayerName").Value == game.Players.LocalPlayer.Name then
                if tonumber((v.BalloonRoot.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
...
function getflower()
    ...
    if tonumber((flowerrrr-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) <= temptable.magnitude/1.4 and tonumber((flowerrrr-fieldposition).magnitude) <= temptable.magnitude/1.4 then 
        if temptable.running == false then 
            if sleepy.toggles.loopfarmspeed then 
                game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = sleepy.vars.farmspeed 
            end 
...
function getcloud()
    for i,v in next, game:GetService("Workspace").Clouds:GetChildren() do
        ...
        if e and tonumber((e.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
...
function getcoco(v)
    ...
    sleepy.tween(.1, v.CFrame)
    repeat task.wait() sleepy.walkTo(v.Position) until not v.Parent
    task.wait(.1)
...
function getfuzzy()
    pcall(function()
        for i,v in next, game.workspace.Particles:GetChildren() do
            if v.Name == "DustBunnyInstance" and temptable.running == false and tonumber((v.Plane.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
...
function getflame()
    for i,v in next, game:GetService("Workspace").PlayerFlames:GetChildren() do
        if tonumber((v.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude) < temptable.magnitude/1.4 then
            farm(v)
            break
        end
    end
...
function avoidmob()
    for i,v in next, game:GetService("Workspace").Monsters:GetChildren() do
        if v:FindFirstChild("Head") then
            if (v.Head.Position-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude < 30 and sleepy.humanoid():GetState() ~= Enum.HumanoidStateType.Freefall then
                game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.Jump = true
            end
        end
    end
end
...
function getcrosshairs(v)
    ...
    sleepy.walkTo(v.Position)
    repeat task.wait() sleepy.walkTo(v.Position) until not v.Parent or v.BrickColor == BrickColor.new("Forest green")
    task.wait(.1)
        ...
        if v.Name ~= "Ant Challenge Info" and v.Name ~= "Bubble Bee Man 2" and v.Name ~= "Wind Shrine" and v.Name ~= "Gummy Bear" then if v:FindFirstChild("Platform") then if v.Platform:FindFirstChild("AlertPos") then if v.Platform.AlertPos:FindFirstChild("AlertGui") then if v.Platform.AlertPos.AlertGui:FindFirstChild("ImageLabel") then
            ...
            if image.ImageTransparency == 0 then
                if sleepy.toggles.tptonpc then
                    game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(v.Platform.Position.X, v.Platform.Position.Y+3, v.Platform.Position.Z)
                    task.wait(1)
                else
...
wayp:CreateDropdown("Field Teleports", fieldstable, function(Option) game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").FlowerZones:FindFirstChild(Option).CFrame end)
wayp:CreateDropdown("Monster Teleports", spawnerstable, function(Option) d = game:GetService("Workspace").MonsterSpawners:FindFirstChild(Option) game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(d.Position.X, d.Position.Y+3, d.Position.Z) end)
wayp:CreateDropdown("Toys Teleports", toystable, function(Option) d = game:GetService("Workspace").Toys:FindFirstChild(Option).Platform game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(d.Position.X, d.Position.Y+3, d.Position.Z) end)
wayp:CreateButton("Teleport to hive", function() game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.SpawnPos.Value end)


local miscc = misctab:CreateSection("Misc")
miscc:CreateButton("Ant Challenge Semi-Godmode", function() sleepy.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) task.wait(1) game.ReplicatedStorage.Events.ToyEvent:FireServer("Ant Challenge") game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position = Vector3.new(93.4228, 42.3983, 553.128) task.wait(2) game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.Name = 1 local l = game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character["1"]:Clone() l.Parent = game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character l.Name = "Humanoid" task.wait() game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character["1"]:Destroy() sleepy.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) task.wait(8) sleepy.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) end)
...
misco:CreateButton("Export Stats Table", function() local StatCache = require(game.ReplicatedStorage.ClientStatCache)writefile("Stats_"..sleepy.nickname..".json", StatCache:Encode()) end)
...
extras:CreateButton("Invisibility", function(State) sleepy.teleport(CFrame.new(0,0,0)) wait(1) if game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character:FindFirstChild('LowerTorso') then Root = game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.LowerTorso.Root:Clone() game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.LowerTorso.Root:Destroy() Root.Parent = game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.LowerTorso sleepy.teleport(game:GetService("Players").LocalPlayer.SpawnPos.Value) end end)
...
task.spawn(function() while task.wait() do
    if sleepy.toggles.autofarm then
        ...
        if game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character:FindFirstChild("ProgressLabel",true) then
        local pollenprglbl = game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character:FindFirstChild("ProgressLabel",true)
        ...
        if sleepy.toggles.farmpuffshrooms and game.Workspace.Happenings.Puffshrooms:FindFirstChildOfClass("Model") then 
            ...
            elseif sleepy.partwithnamepart("Legendary", game.Workspace.Happenings.Puffshrooms) then
                temptable.magnitude = 25 
                fieldpos = sleepy.partwithnamepart("Legendary", game.Workspace.Happenings.Puffshrooms):FindFirstChild("Puffball Stem").CFrame
                fieldposition = fieldpos.Position
            elseif sleepy.partwithnamepart("Epic", game.Workspace.Happenings.Puffshrooms) then
                temptable.magnitude = 25 
                fieldpos = sleepy.partwithnamepart("Epic", game.Workspace.Happenings.Puffshrooms):FindFirstChild("Puffball Stem").CFrame
                fieldposition = fieldpos.Position
            elseif sleepy.partwithnamepart("Rare", game.Workspace.Happenings.Puffshrooms) then
                temptable.magnitude = 25 
                fieldpos = sleepy.partwithnamepart("Rare", game.Workspace.Happenings.Puffshrooms):FindFirstChild("Puffball Stem").CFrame
                fieldposition = fieldpos.Position
            else
                temptable.magnitude = 25 
                fieldpos = sleepy.getbiggestmodel(game.Workspace.Happenings.Puffshrooms):FindFirstChild("Puffball Stem").CFrame
                fieldposition = fieldpos.Position
            end
        end
        if tonumber(pollenpercentage) < tonumber(sleepy.vars.convertat) then
            if not temptable.tokensfarm then
                ...
                if sleepy.toggles.killmondo then
                    while sleepy.toggles.killmondo and game.Workspace.Monsters:FindFirstChild("Mondo Chick (Lvl 8)") and not temptable.started.vicious and not temptable.started.monsters do
                        ...
                        while game.Workspace.Monsters:FindFirstChild("Mondo Chick (Lvl 8)") do
                            ...
                            sleepy.tween(1, CFrame.new(mondopition.x, mondopition.y - 60, mondopition.z))
                            task.wait(1)
                            temptable.float = true
                        end
                        task.wait(.5) game:GetService("Workspace").Map.Ground.HighBlock.CanCollide = true temptable.float = false sleepy.tween(.5, CFrame.new(73.2, 176.35, -167)) task.wait(1)
                        for i = 0, 50 do 
                ...
                if (fieldposition-game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).magnitude > temptable.magnitude then
                    sleepy.tween(2, fieldpos)
                    task.wait(2)
                    if sleepy.toggles.autosprinkler then makesprinklers() end
                end
			...
			for i,v in next, game.workspace.Particles:GetChildren() do
				for x in string.gmatch(v.Name, "Vicious") do
					if string.find(v.Name, "Vicious") then
						sleepy.tween(1,CFrame.new(v.Position.x, v.Position.y, v.Position.z)) task.wait(1)
						sleepy.tween(0.5, CFrame.new(v.Position.x, v.Position.y, v.Position.z)) task.wait(.5)
					end
				end
			end
			for i,v in next, game.workspace.Particles:GetChildren() do
            ...
            if aw then
                for i,v in pairs(workspace.Monsters:GetChildren()) do
                    if string.find(v.Name, "Windy") then
                        if v.Name ~= wlvl then
                            temptable.float = false task.wait(5) for i =1, 5 do gettoken(sleepy.humanoidrootpart().Position) end -- collect tokens :yessir:
                            wlvl = v.Name
                        end
                    end
                end
            end
            if not awb then sleepy.tween(1,temptable.gacf(temptable.windy, 5)) task.wait(1) awb = true end
            if awb and temptable.windy.Name == "Windy" then
                sleepy.humanoidrootpart().CFrame = temptable.gacf(temptable.windy, 25) temptable.float = true task.wait()
            end
        end 
        enableall()
        temptable.float = false
...
task.spawn(function() while task.wait(0.001) do
    if sleepy.toggles.traincrab then game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-259, 111.8, 496.4) * CFrame.fromEulerAnglesXYZ(0, 110, 90) temptable.float = true temptable.float = false end
    if sleepy.toggles.farmrares then for k,v in next, game.workspace.Collectibles:GetChildren() do if v.CFrame.YVector.Y == 1 then if v.Transparency == 0 then decal = v:FindFirstChildOfClass("Decal") for e,r in next, sleepy.rares do if decal.Texture == r or decal.Texture == "rbxassetid://"..r then game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = v.CFrame break end end end end end end
    if sleepy.toggles.autodig then workspace.NPCs.Onett.Onett["Porcelain Dipper"].ClickEvent:FireServer() if game.Players.LocalPlayer then if game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character then if game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character:FindFirstChildOfClass("Tool") then if game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character:FindFirstChildOfClass("Tool"):FindFirstChild("ClickEvent", true) then clickevent = game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character:FindFirstChildOfClass("Tool"):FindFirstChild("ClickEvent", true) or nil end end end if clickevent then clickevent:FireServer() end end end
end end)
        ...
        if sleepy.toggles.autosamovar then
            ...
            for i,v in pairs(game.Workspace.Collectibles:GetChildren()) do
                if (v.Position-platformm.Position).magnitude < 25 and v.CFrame.YVector.Y == 1 then
                    sleepy.humanoidrootpart().CFrame = v.CFrame
                end
            end
        end
...
game:GetService('RunService').Heartbeat:connect(function() 
    ...
    if sleepy.toggles.loopspeed then game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = sleepy.vars.walkspeed end
    if sleepy.toggles.loopjump then game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = sleepy.vars.jumppower end
end)
...
game:GetService('RunService').Heartbeat:connect(function() 
    if temptable.float then game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.Humanoid.BodyTypeScale.Value = 0 floatpad.CanCollide = true floatpad.CFrame = CFrame.new(game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position.X, game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position.Y-3.75, game.Players.LocalPlayer.game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position.Z) task.wait(0)  else floatpad.CanCollide = false end
end)
    ...
    if sleepy.toggles.farmsnowflakes then
        ...
        for i,v in next, temptable.tokenpath:GetChildren() do
            if v:FindFirstChildOfClass("Decal") and v:FindFirstChildOfClass("Decal").Texture == "rbxassetid://6087969886" and v.Transparency == 0 then
                sleepy.humanoidrootpart().CFrame = CFrame.new(v.Position.X, v.Position.Y+3, v.Position.Z)
                break
            end
        end
    end
